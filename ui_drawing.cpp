...
void DrawCharacterStatusUI(const Character& c, int x, int y, bool isTurn) {
    ...
    for (int i = 0; i < c.inventory.size(); i++) {
        DrawText(TextFormat("[%d] %s (%s)", i + 1, c.inventory[i].name.c_str(), c.inventory[i].classification.c_str()), x + 10, statusOffsetY + i * 20, 16, GRAY);
    }
}
    ...
    if (!gameOver) {
        if (showItemChoice) {
            if (choosingSwapOrDiscard) { 
                std::string invFullMsg = currentPlayer.name + " found: " + currentFoundItem.name + " ("+ currentFoundItem.classification +"). Inventory FULL!";
                ...
                std::string foundItemMsgText = currentPlayer.name + " found: " + currentFoundItem.name + " ("+ currentFoundItem.classification +")";
            ...
            if (currentPlayer.inventory.size() >= 1) {
                ...
                DrawText(TextFormat("Use: %s (%s)", currentPlayer.inventory[0].name.c_str(), currentPlayer.inventory[0].classification.c_str()), btnChooseInvItem1.x + 20, btnChooseInvItem1.y + 10, 20, BLACK);
            }
            if (currentPlayer.inventory.size() >= 2) {
                DrawRectangleRec(btnChooseInvItem2, SKYBLUE);
                DrawText(TextFormat("Use: %s (%s)", currentPlayer.inventory[1].name.c_str(), currentPlayer.inventory[1].classification.c_str()), btnChooseInvItem2.x + 20, btnChooseInvItem2.y + 10, 20, BLACK);
            }
            if (currentPlayer.inventory.size() >= 3) {
                DrawRectangleRec(btnChooseInvItem3, SKYBLUE);
                DrawText(TextFormat("Use: %s (%s)", currentPlayer.inventory[2].name.c_str(), currentPlayer.inventory[2].classification.c_str()), btnChooseInvItem3.x + 20, btnChooseInvItem3.y + 10, 20, BLACK);
            }
